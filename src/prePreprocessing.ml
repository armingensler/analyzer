...
let get_line_types lines pp_vars =
    ...
    let get_line_type line pp_vars =
        ...
        if not (starts_with trimmed "#") then 
            ...
            if starts_with trimmed "ifdef" then
...
let get_replacements lines =

    let rec impl rest result stack = 
        match rest with
        | [] -> result
        ...
        | ((LT_FLAG_IFDEF (v, l1, l2), _) :: rest) ->
            ...
            impl rest (r :: result) (SE_VARIF :: stack)
        ...
        | ((LT_PP_IF, line) :: rest) -> 
            impl rest result (SE_IF :: stack)
            
        | ((LT_PP_ELIF, line) :: rest) -> 
            begin match stack with
            | [] -> impl rest result []
            | (SE_IF :: _) -> impl rest result stack
...
let pre_preprocess infile outfile ppvars =
    ...
    if get_bool "dbg.verbose" then print_endline ("  in_filename = " ^ infile);
    if get_bool "dbg.verbose" then print_endline ("  out_filename = " ^ outfile);
    ...
    let replacements = get_replacements with_types in
